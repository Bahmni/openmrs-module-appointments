...
VALUES (
           ...
           'SELECT
               ...
               TO_CHAR(start_date_time, ''DD/MM/YYYY'') AS "DASHBOARD_APPOINTMENTS_DATE_KEY",
               TO_CHAR(start_date_time, ''HH12:MI AM'') || '' - '' || TO_CHAR(end_date_time, ''HH12:MI AM'') AS "DASHBOARD_APPOINTMENTS_SLOT_KEY",
               pn.given_name || '' '' || pn.family_name AS "DASHBOARD_APPOINTMENTS_PROVIDER_KEY",
           ...
           JOIN appointment_service app_service ON app_service.appointment_service_id = pa.appointment_service_id AND app_service.voided IS FALSE
           LEFT JOIN patient_appointment_provider pap ON pa.patient_appointment_id = pap.patient_appointment_id AND (pap.voided IS FALSE OR pap.voided IS NULL)
           ...
           WHERE p.uuid = ${patientUuid}
             ...
             AND (app_service_type.voided IS FALSE OR app_service_type.voided IS NULL)
